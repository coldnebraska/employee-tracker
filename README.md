# employee-tracker [![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT) 

## Project Description
This project wa designed to help me learn how to connect and update a mysql database. I used mysql2 to connect to the database and used inquirer to prompt the user for directions on what info was needed to be returned or updated.

## Table of Contents
[Installation](#installation)  
[Usage](#usage)   
[License](#license)  
[Questions](#questions)  

## Installation
This repository can be cloned or downloaded as a zip file.

## Usage
Please refer to the included usage video.  
https://drive.google.com/file/d/1538UvSzyQljEkE6vxNLEm_AyliK5hCoT/view

## License
MIT License

## Questions
Github: https://github.com/coldnebraska  
Email: brush.david99@gmail.com
